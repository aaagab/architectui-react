...
import { formatValue, getFilledLength, isFilled, isEmpty, getRightEditablePosition, getStringValue } from "./utils/string";
...
class InputElement extends React.Component {
    ...
    constructor(props) {
        ...
        this.value = newValue;
        this.nodeRef = props.inputRef || React.createRef();
    }
    ...
    getInputDOMNode = () => {
        ...
        let input = this.nodeRef.current;
    ...
    render() {
        ...
        if (children) {
            invariant(isFunction(children), "react-input-mask: children must be a function");
            ...
            const conflictProps = controlledProps.filter((propId) => inputElement.props[propId] != null && inputElement.props[propId] !== restProps[propId]);
            ...
            warning(!inputRef, "react-input-mask: inputRef is ignored when children is passed, attach ref to the children instead");
        } else {
            // inputElement = <input ref={this.handleRef} {...restProps} />;
            inputElement = <input ref={this.nodeRef} {...restProps} />;
        }
